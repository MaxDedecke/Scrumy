...
- Architektur-Standard, kein Vorschlag: Die Software läuft als Docker-Compose-Umgebung mit einem eigenen Container je Dienst (Microservice-Zuschnitt statt Monolith, z.B. getrennte Container für Frontend, Backend/API und Datenbank). Gibt es ein Frontend, bekommt es einen eigenen Container in einem konventionellen Verzeichnis ('frontend/', 'web/', 'client/', 'app/', 'ui/' oder Projektwurzel) mit einem 'dev'-, 'start'- oder 'preview'-Skript in dessen package.json – daran erkennt und startet Scrumys Vorschau-Funktion es automatisch. Weiche davon nur ab, wenn (a) Konzept, Anforderungen oder ein Beschluss des Auftraggebers ausdrücklich etwas anderes verlangen, (b) das Projekt kein Frontend hat (reiner Service/reine API – dann ist Docker optional), oder (c) bestehender Code importiert wird, der schon eine andere Struktur mitbringt. Sonst gilt dieser Zuschnitt, ohne dass ihn jemand extra anfordern muss.
